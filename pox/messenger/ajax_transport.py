...
class AjaxConnection (Connection):
  ...
  def tx (self, wfile, seq, batch_size):
    ...
    with self._cond:
      data = self._get_tx_batch(seq = seq, batch_size = batch_size) #empty data
      if len(data) == 0:
        # Wait for messages
        ...
        while True:
          ...
          self._cond.wait(2)
          data = self._get_tx_batch(seq = seq, batch_size = batch_size) #get data
          #print '01 self._get_tx_batch(seq = seq, batch_size = batch_size)',self._get_tx_batch(seq = seq, batch_size = batch_size)
          if len(data) > 0:
            # See if we can get a bit more data
            self._cond.wait(.05)
            data = self._get_tx_batch(seq = seq, batch_size = batch_size)
            #print '02 self._get_tx_batch(seq = seq, batch_size = batch_size)',self._get_tx_batch(seq = seq, batch_size = batch_size)
            break
  ...
  def rx (self, msg, seq):
    ...
    def do_rx (msg):
      if isinstance(msg, list):
        for m in msg:
          self._rx_message(m)
          #print 'self._rx_message:',self._rx_message
      else:
...
class AjaxMsgHandler (JSONRPCHandler):
  ...
  def _exec_poll (self, session_id, seq = None, batch_size = None):
    ...
    #print 'self.wfile',self.wfile #socket file
    #print 'ses.tx',ses.tx,'r',r
    if isinstance(r, dict) and 'result' in r:
      r['result']['session'] = ses._session_id
    return r
