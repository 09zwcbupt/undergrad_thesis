...
qx.Class.define("poxdesk.Messenger",
{
  ...
  construct : function()
  {
	//myvalue=prompt("ip of controller:");
    var rpc = new qx.io.remote.Rpc("/_jrpcmsg/", "messenger_tx");
	//var rpc = new qx.io.remote.Rpc( "http://" + myvalue + ":8000/_jrpcmsg/", "messenger_tx");
    ...
    this._rx_requests = {};
    this._channels    = {};
    this._channel_rc  = {}; // Refcounts (join counts)
    
    rpc = new qx.io.remote.Rpc("/_jrpcmsg/", "messenger_rx");
	//rpc = new qx.io.remote.Rpc( "http://" + myvalue + ":8000/_jrpcmsg/", "messenger_rx");
	//rpc.setCrossDomain(true);
    //this.debug("rpc working2");
    this._rxer = rpc;
  ...
  members :
  {
    ...
    {
      this._dead = true;
      this._txer.callAsync(function () {}, "stop", this._ses_key);
    ...
    send : function (_data, channel)
    {
      ...
      qx.lang.Object.carefullyMergeWith(data, _data);
    ...
    leave : function (channel)
    {
      ...
      var m = {'CHANNEL':'','cmd':'leave_channel','channel':channel};
      this.send(m);
    },
    ...
    join : function (channel, extra)
    {
      ...
      this._channel_rc[channel] = c;
      if (c != 1) return;
      var m = {'CHANNEL':'','cmd':'join_channel','channel':channel};
      if (extra) qx.lang.Object.carefullyMergeWith(m, extra);
      this.send(m);
    },
